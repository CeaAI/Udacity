...
zSetDecodedEntriesStreamingDF1= zSetEntriesEncodedStreamingDF.withColumn("customer", unbase64(zSetEntriesEncodedStreamingDF.encodedCustomer).cast("string"))
...
customerWithDetailsDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr("""
    email = customer
"""                                                                                 
))
...
query1 = customerWithDetailsDF.selectExpr("cast(customer as string) as key", "to_json(struct(*)) as value") \
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("topic", "stedi-graphs")\
    .option("checkpointLocation","/tmp/kafka-checkpoint-3")\
.option("failOnDataLoss", "false") \
    .start()

query2 = customerWithDetailsDF.writeStream.outputMode("append").format("console").start()
